...
def get_name_industry(stock):
	...
	company_data = {'name': results['companyName'], 'sector': results['sector']}
	return(company_data)
...
def download_stock(stock):
	...
	try:
		...
		stock_df['sector'] = company_name_and_industry['sector']
		...
		stock_df = stock_df[['date', 'name', 'symbol', 'sector', 'open', 'high', 'low', 'close', 'volume']]
...
if __name__ == '__main__':
	...
	max_workers = 50
if (len(sys.argv) <= 1):
	print("\n\nUsage: python getSandP.py [ -all_stocks | -sp [list of values company symbols to download]]\n\n")
	print("	-sp: download historical data for all companies in S&P 500")
	print("	-list [symbols]: download historical data for all companies in list")
	print("		ex. python getSandP.py -list aapl msft\n\n") 
	...
	workers = min(max_workers, len(sys.argv) - 1) #in case a smaller number of stocks than threads was passed in
	with futures.ThreadPoolExecutor(workers) as executor:
		res = executor.map(download_stock, sys.argv[2:])
